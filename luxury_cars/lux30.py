...
for i in range(len(title)):
    ...
    empty = []
    while True:
        url = 'https://www.automobile.fr/voiture/{}/vhc:car,pgn:{},pgs:30,ms1:{}__,dmg:false'.format(title[i],start,val[i])
        r = s.get(url)
    ...
    rest_page_30.append(empty)
    

for j in range(len(title)):
    dict1 = {title[j]: rest_page_30[j]}
    df = pd.DataFrame(dict1)
    f = open('lux30.csv',mode='a',encoding='utf-8')
    df.to_csv(f)
